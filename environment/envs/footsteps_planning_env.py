...
from environment.simulator.simulator import Simulator as FootstepsSimulator
from environment.simulator import transform as tr
...
class FootstepsPlanningEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        self.panjang = options.get("panjang", 3)
        self.lebar = options.get("lebar", 3)
    ...
    def reset(self, seed: int | None = None, options: Optional[dict] = None):
        ...
        if start_foot_pose is None:
            start_foot_pose = self.np_random.uniform([0, 0, -math.pi], [self.panjang, self.lebar, math.pi])

        # Initializing the simulator
        self.simulator.init(*start_foot_pose, start_support_foot)

        if self.target_foot_pose is None:
            self.target_foot_pose = self.np_random.uniform([0, 0, -math.pi], [self.panjang, self.lebar, math.pi])
